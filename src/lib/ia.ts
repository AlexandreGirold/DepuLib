...
export type FeedOut = { items: FeedItemOut[]; sources: Source[] };

export async function genereFeed(
  profil: { circonscription?: string | null; commentesTitres: string[] },
  dossiers: { titre: string; expose: string; sourceUrl: string }[]
): Promise<FeedOut> {
  const allowed = dossiers.map((d) => d.sourceUrl);
  ...
  const ctx = dossiers
    .map((d) => `- « ${d.titre} » (${d.sourceUrl}) : ${d.expose.slice(0, 400)}`)
    .join("\n");
  const raw = await chat(
    [
      sys(
        "Génère un feed d'actualité législative personnalisé, ton sobre et " +
          "pédagogique (style « décryptage » mesuré, sans sensationnalisme ni " +
          "opinion). Chaque item cite une source officielle parmi celles fournies. " +
          'Format : {"items": [{"titre": string accrocheur, "resume": string ' +
          '(2 phrases), "lien": string (une URL officielle fournie), "tag": ' +
          'string thématique}], "sources": [{"url","titre"}]}. 3 à 5 items.'
      ),
      {
        role: "user",
        content:
          `Profil citoyen — circonscription : ${profil.circonscription ?? "n.c."}. ` +
          `Dossiers déjà commentés : ${profil.commentesTitres.join(", ") || "aucun"}.\n\n` +
          `Dossiers disponibles :\n${ctx}`
      }
    ],
    { model: MODELS.main, json: true, maxTokens: 1200, mockKey: `feed:${hashKey((profil.circonscription ?? "") + profil.commentesTitres.join(","))}` }
  );
  const parsed = parseJsonLoose<any>(raw);
  if (parsed && Array.isArray(parsed.items)) {
    const allowedSet = new Set(allowed.map((u) => u.replace(/\/+$/, "")));
    const items: FeedItemOut[] = parsed.items
      .map((it: any) => ({
        titre: String(it.titre ?? ""),
        resume: String(it.resume ?? ""),
        lien: String(it.lien ?? ""),
        tag: String(it.tag ?? "Actualité")
      }))
      .filter((it: FeedItemOut) => allowedSet.has(it.lien.replace(/\/+$/, "")))
      .slice(0, 5);
    if (items.length > 0) {
      return { items, sources: validateSources(parsed.sources, allowed) };
    }
  }
  // Fallback : items dérivés directement des dossiers
  const items = dossiers.slice(0, 4).map((d) => ({
    titre: d.titre,
    resume: d.expose.slice(0, 160).replace(/\s+\S*$/, "") + "…",
    lien: d.sourceUrl,
    tag: "Législation"
  }));
  return { items, sources: dossiers.slice(0, 4).map((d) => ({ url: d.sourceUrl, titre: d.titre })) };
}
